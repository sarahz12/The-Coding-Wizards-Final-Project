...
def get_piano_notes_2():
    ...
    note_freqs_2 = {all_notes[i]: base_freq * pow(2,(i/12)) for i in range(len(all_notes))} # indexing all of the notes and giving them the given frequency    
    note_freqs_2[''] = 0.0 # silent note
    
    return note_freqs_2
  
  # To retrieve the piano note's frequencies
note_freqs_2 = get_piano_notes_2()


from scipy.io.wavfile import write
# Lines 53-65: take the frequency values for a specific note and assigns it to a variable
# For example, A3 = the frequency for A3 from the numpy array
# Frequency values generated by get_piano_notes
...
# Takes the frequency value from the numpy array, stored as key name (e.g., A3) and converts to a wav file
...
def piano_gui():
    ...
    pygame.draw.rect(screen,white,(525,200 +100,50,75)) # The +100 indicates the rectangle starts lower than y_coord
...
def auto_play():
    ...
    prevNote = '' # space between each note
    ...
    for note in twinkle_song:
        if note == '': # rest note
            ...
            # D and B have their own line because they are connected to the black keys
            if note == 'D': 
        ...
        pygame.display.update() # update dictionary
...
while running:
    for events in pygame.event.get():
        ...
        if events.type == pygame.QUIT:
            pygame.quit() # exits the system
            sys.exit()
            quit()
            
    screen.fill((0,100,130)) # fill numpy array with scalar value
    screen.blit(background, (0, 0)) # block transfer; positions background image
    largeText = pygame.font.Font('freesansbold.ttf',115) # font size of text displayed
    TextSurf, TextRect = text_objects("Piano", largeText) # displays the word 'piano' 
